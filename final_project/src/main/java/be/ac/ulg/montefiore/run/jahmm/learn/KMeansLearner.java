...
{
	public Clusters<O> clusters;
	public int nbStates;
	public List<? extends List<? extends O>> obsSeqs;
	...
	public void learnAij(Hmm<O> hmm)
	{
	...
	public void learnOpdf(Hmm<O> hmm)
	{
		for (int i = 0; i < hmm.nbStates(); i++) {
	...
	public boolean optimizeCluster(Hmm<O> hmm)
	{
		boolean modif = false;
	...
	public static ArrayList<ObservationVector> flat (List<List<ObservationVector>> lists, int garbage) {
		ArrayList<ObservationVector> v = new ArrayList<>();
		for (List<ObservationVector> list : lists)
			v.addAll(list);
		return v;
	}
}
...
class Clusters<O extends CentroidFactory<? super O>>
{
	...
	public Hashtable<O,Value> clustersHash;
	public ArrayList<Collection<O>> clusters;

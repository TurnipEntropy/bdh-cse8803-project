create or replace function get_eye_opening_table()
returns table(subject_id integer, icustay_id integer, 
              charttime timestamp without time zone, eye_opening double precision)
as '
  select subject_id, icustay_id, charttime, (combo).value from(
select subject_id, icustay_id, charttime, eye_opening_aggregate(itemid, avg_value) as combo from(
  select subject_id, icustay_id, charttime, itemid, avg(valuenum) as avg_value from (
    ...
    and ce.itemid in (220739,226756)
   ) t1 

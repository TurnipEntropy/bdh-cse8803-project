create or replace function get_motor_table()
returns table(subject_id integer, icustay_id integer, 
              charttime timestamp without time zone, motor double precision)
as '
  select subject_id, icustay_id, charttime, (combo).value from(
select subject_id, icustay_id, charttime, motor_aggregate(itemid, avg_value) as combo from(
  select subject_id, icustay_id, charttime, itemid, avg(valuenum) as avg_value from (
    ...
    and ce.itemid in (223901,226757)
   ) t1 

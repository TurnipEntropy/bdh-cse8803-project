create or replace function get_temperature_table()
returns table(subject_id integer, icustay_id integer, 
              charttime timestamp without time zone, temp_F double precision)
as '
  select subject_id, icustay_id, charttime, (combo).value 
  from(
    select subject_id, icustay_id, charttime, temp_aggregate(itemid, avg_value) as combo 
    from(
      ...
      from (
        ...
        and ce.itemid in (223762,223761)
      ) t1 

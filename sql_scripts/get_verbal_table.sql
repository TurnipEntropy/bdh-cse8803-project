create or replace function get_verbal_table()
returns table(subject_id integer, icustay_id integer, 
              charttime timestamp without time zone, verbal double precision)
as '
  select subject_id, icustay_id, charttime, (combo).value from(
select subject_id, icustay_id, charttime, verbal_aggregate(itemid, avg_value) as combo from(
  select subject_id, icustay_id, charttime, itemid, avg(valuenum) as avg_value from (
    ...
    and ce.itemid in (223900,226758)
   ) t1 
